...
public class CubeTransitionInfiniteView: UIView {
    ...
    public var offsetCachedPageNumber: Int = 1
    ...
    private var maxRotateAngleForView: CGFloat = CGFloat(Double.pi / 3)
    ...
    private func initGesture () {
        if gestureDistanceForPageFlipping == 0 {
            gestureDistanceForPageFlipping = frame.width / 2
        }
    ...
    func maintainCacheSize() {
        ...
        if (currentIndex < offsetCachedPageNumber || currentIndex >= maxIndex) { return }
    ...
    func setContinousDistanceAfterGestureEnded(tx: CGFloat) {
        if (shouldChangePage) {
            ...
            continuousDistance = tx > 0 ? frame.width - tx : -frame.width - tx;
        } else {
    ...
    func updateBoundsAndTransform() {
        ...
        if (shouldChangePage) {
            if (direction == .left) {
                originX = bounds.origin.x - frame.width;
            } else if (direction == .right){
                originX = bounds.origin.x + frame.width;
            }
        }
    ...
    func animtedViewsBy(translation: CGPoint) {
        let k = translation.x / frame.width;
        if (translation.x > 0) {
            // Scroll to the left side
            // 0 -> -angle
            ...
            let gap1 =  frame.width / 2.0 * (1-cos(r));
            let gap2 = frame.width / 2.0 * (1-cos(r2));
            
            leftView.layer.transform = self.transformFrom(rotate:r, translateX:translation.x + gap1, translateZ:sin(r) * frame.width/2);
            rightView.layer.transform = self.transformFrom(rotate:r2, translateX:translation.x - gap2, translateZ:-sin(r2) * frame.width/2);
        } else if (translation.x < 0) {
            // scroll right: from 0 --> - M_PI/4
            let r =  k * maxRotateAngleForView;
            let gapWidth1 = frame.width / 2.0 * (1-cos(r));
            
            // sin(r) < 0
            leftView.layer.transform = self.transformFrom(rotate:r, translateX:translation.x + gapWidth1, translateZ:sin(r) * frame.width / 2);
            
            // The angle between the two views should be fixed
            // Angle -> 0
            let r2 = (maxRotateAngleForView + r);
            let gapWidth2 = frame.width / 2.0 * (1-cos(r2));
            // sin(r2) > 0
            rightView.layer.transform = self.transformFrom(rotate:r2, translateX:translation.x - gapWidth2, translateZ:-sin(r2) * frame.width / 2);
        }
    }
